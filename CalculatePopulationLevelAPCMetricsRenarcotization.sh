...
#SBATCH -J CPLAPCMRN								# Name the job as 'MPItest'
#SBATCH --account=CDERID0047
#SBATCH --output=output/logfiles/CPLAPCMRN_%j.out	# Write the standard output to file
...
Rscript calculatePopulationLevelAPCMetric.R -b "$opioidDose" -c "$antagonist" -d "$antagonistAdministrationRouteAndDose"	-r "$inputDate"	-s "$simulateRenarcotization" -t "$PercentCI" -u "$typicalSubjectInputDate" -v "$opioidDosingRoute" -w "$fractionOfBaselineVentilationForAntagonistAdministration"