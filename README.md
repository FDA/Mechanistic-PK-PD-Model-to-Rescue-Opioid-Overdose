...
Binding paramters (Kon, Koff and n) for naloxone, the opioid ligands used in clinical simulations, and carfentanil can be found in their corresponding folder in [Ligand_Data/](https://github.com/FDA/Mechanistic-PK-PD-Model-to-Rescue-Opiod-Overdose/tree/main/Ligand_Data). Using Fentanyl as an example, optimal parameters are saved in the [pars.txt](https://github.com/FDA/Mechanistic-PK-PD-Model-to-Rescue-Opiod-Overdose/blob/main/Ligand_Data/Fentanyl_A_parameters/Fentanyl_A_pars.txt) file and [boot_pars.csv](https://github.com/FDA/Mechanistic-PK-PD-Model-to-Rescue-Opiod-Overdose/blob/main/Ligand_Data/Fentanyl_A_parameters/boot_pars.csv) contains the uncertainty population for these binding parameters. The file [parmsAlgera2020.R](https://github.com/FDA/Mechanistic-PK-PD-Model-to-Rescue-Opiod-Overdose/blob/main/Ligand_Data/Fentanyl_A_parameters/parmsAlgera2020.R) contains the full set of binding and PK parameters necessary for simulating opioid overdose and subsequent rescue using intranasal naloxone for the optimal scenario. 
...
The modeling strategy above enabled us to predict the ventilation response after 0.035 mg remifentanil IV bolus injection reasonably well. [Remifentanil_Prediction]( https://github.com/FDA/Mechanistic-PK-PD-Model-to-Rescue-Opiod-Overdose/blob/main/Clinical_Comparison_all/Clinical_Comparison_Remifentanil/figs/Naive_Remifentanil_Ventilation.pdf)

# Simulate Real World Respiratory Depression for Fentanyl or Carfentanil Overdose Scenarios 
## Important Modeling Considerations

It is prudent to realize that the model described by the equations above simulates steady-state isohypercapnic ventilation, when the end-tidal CO2 is fixed at a high level in a clinical study so that the baseline minute ventilation volume is stable and substantially higher (20 L/min, 13 L/min, etc.) than the normal minute ventilation volume (5 – 8 L/min). Other than this, there are important differences between the model and real-world situations when breathing room air. First of all, the endpoint of the model,  as well as the underline clinical studies, is isohypercapnic ventilation, when the alveolar and arterial CO2 pressure is high and fixed (in a clinical study). In real-world poikilocapnic situation, a bolus opioid injection can initially decrease the ventilation significantly, which will increase the alveolar and arterial CO2 pressure, and trigger chemoreflex reaction and increase minute ventilation volume. Second, after the initial ventilation suppression, the O2 content in blood is lowered, and this hypoxic condition can increase the sensitivity of the chemoreflex system to increasing CO2, providing more gain of spontaneous recovery of ventilation. Moreover, there may be a temporary increase in cardiac output after the initial respiratory depression, which can compensate some part of the drop in O2 content. All of these factors, which represent physiological responses to counteract a sudden decrease of ventilation due to a bolus opioid overdose scenario, are missing from the current model.
...
As an attempt to bridge the gap between our model endpoint (isohypercapnic ventilation) and real-world breathing (when both CO2 and O2 are changing), we adopted several strategies. In simulating a bolus fentanyl or carfentanil injection, we modeled the ventilation time course and calculated rescue time, defined as the difference between the first naloxone administration timepoint and when minute ventilation volume first recovered back above a certain rescue threshold. We used three levels as thresholds: 20%, 40%, and 80% of the baseline minute ventilation These different levels of threshold were explored as it is possible that a small recovery in isohypercapnic condition (model endpoint) corresponds to a large recovery in real-world condition (due to spontaneous recovery). We set the baseline ventilation (VB) in equation 18 at 15 L/min (compared to 6-8 L/min at rest) because in a clinical ventilation study at isohypercapnic conditions (end-tidal CO2 fixed at 50 mm Hg, leading to a constant baseline ventilation of 13 L/min) reported a remifentanil half-effect concentration C50 (see equation 14) of 1.12 – 1.36 ng/mL [7], which is slightly lower than the C50 (1.6 ng/mL) reported when remifentanil was given in real-world poikilocapnic situation [6].  We found that the observed degree of respiratory depression is inversely correlated with the fixed baseline ventilation level in isohypercapnic conditions (see equation 18), this suggests simulating an isohypercapnic condition with a fixed baseline ventilation slightly higher than 13L/min may give an apparent C50 closer to the real-world poikilocapnic situation. However, this is still different from the real-world situation, and we are developing more realistic models to account for the spontaneous ventilation recovery due to the change of O2 and CO2 in a real-world situation. 
