...
#define f1 parms[104]
#define f2 parms[105]
#define molarMassAntagonist parms[106]
#define brainWeight parms[107]
#define tau_o2Child parms[108]
#define c1o2Child parms[109]
#define Q_0 parms[110]
#define timeout parms[111]
#define starttime parms[112]

#define opioidMolarMass parms[113]
#define opioidCentralVolume parms[114]
#define opioidPeripheralVolume parms[115]
#define opioidTransferRate parms[116]
#define opioidAbsorptionRate parms[117]
#define opioidClearanceRate parms[118]
#define opioidInterCompartmentalClearanceRate parms[119]
#define opioidKeo parms[120]

double CA_o2_happen=0;
double CA_co2_happen=0;
double* calc_intermediate(double im11,double im12,double im13,double im14,double im15,double im16,double im18,double im19,double im25,double im26,double im27, double im29){
	...
	//	double i_Qb = Qb0*(1+Cim27*(im18+im19))*im25; //adult
	double i_Qb = Qb0*(1+Cim27*(im18+im29))*im25; //added//
...
void derivs (int *neq, double *t, double *y, double *ydot, double *yout, int *ip){
	...
	double* intermediate_list= calc_intermediate(y[11],y[12],y[13],y[14],y[15],y[16],y[18],y[19],y[25],y[26],y[26],y[29]);
	...
	double k31C = k31;
	double VPC = VP;
	double opioidCentralVolumeModified = opioidCentralVolume;
	...
	opioidCentralVolumeModified=opioidCentralVolume/Q_Scale;
	...
	if (  0<history){
		...
		double ytau13[1] = {0};double ytau14[1] = {0};double ytau15[1] = {0};double ytau16[1] = {0};double ytau11[1] = {0};double ytau12[1] = {0};double ytau19[1] = {0};double ytau18[1] = {0};double ytau29[1] = {0};
		...
		int Nout9 = 1;
		static int nr9[1] = {29};
		lagvalue(history, nr9, Nout9, ytau29);
		ytau[9]=ytau29[0];

		double* Plag_intermediate = calc_intermediate(ytau[4],ytau[5],ytau[0],ytau[1],ytau[2],ytau[3],ytau[7],ytau[6],y[25],y[26],y[27],ytau[9]);
	...
	if (0<historyc) {
		...
		int Nout9c = 1;
		static int nr9c[1] = {29};
		double ytau29c[1] = {0};
		lagvalue(historyc, nr9c, Nout9c, ytau29c);
		//				y[19]=ytau7c[0];
		ytauc[9]=ytau29c[0];

		double* Clag_intermediate = calc_intermediate(ytauc[4],ytauc[5],ytauc[0],ytauc[1],ytauc[2],ytauc[3],ytauc[7],ytauc[6],y[25],y[26],y[27],ytauc[9]);
	...
	double psai_co2 = ((Aco2+Bco2/(1+exp(-(P_a_co2 - Cco2)/Dco2)))*brainWeight/100/1000/60 + Qb0)/Qb0 -1;
	double ReactionFlux24=1.00/tau_co2*(psai_co2 - y[18]);

	double psai_o2 = c1o2*(exp(-P_a_o2/c2o2)- exp(-P_a_o2_0/c2o2));
	double psai_o2Child = c1o2Child*(exp(-P_a_o2/c2o2)- exp(-P_a_o2_0/c2o2)); //unchanged//

	if (psai_o2>=2 ) {
		psai_o2=2;
		//	y[19]=2;
	}
	if (psai_o2Child>=2 ) {//added//
		psai_o2Child=2;//added//
		//	y[19]=2;
	}//added//

	double ReactionFlux25=1.00/tau_o2*(psai_o2 - y[19]);
	double ReactionFlux25Child=1.00/tau_o2Child*(psai_o2Child - y[29]); //unchanged//
	...
	//Antagonist PK (Method 2)===================================================
	ydot[0] = -(ktr*y[30]*y[0])-((1-y[30])*ktr*y[0]); //(in ng/s) given y[0] is (in ng), ktr is (in 1/s), y[30] is (in -)
	ydot[1] = (ktr*y[30]*y[0])-(ktr*y[1]); //(in ng/s) given y[1] is (in ng), ktr is (in 1/s), y[30] is (in -)
	ydot[2] = -(1*kin*y[2])+(ktr*y[1]); //(in ng/s) given y[1] & y[2] are (in ng), kin & ktr are (in 1/s)
	ydot[3] = (1*kin*y[2])-((kout2/V1)*y[3])- ((k12N/V1)*y[3])+((k12N/V2)*y[28]); //(in ng/s) given y[2], y[3] & y[28] are (in ng), kin is (in 1/s), V1 & V2 are (in ml), kout2, k12N are (in ml/s)
	ydot[4] = (k2*((y[3]*1e3)/(V1C/1e3))/molarMassAntagonist)-(k2*y[4]); //(in pM/s) given y[4] is (in pM), k2 is (in 1/s), y[3] is (in ng), V1C is (in ml), molarMassAntagonist is (in g/mol)
	ydot[28] = ((k12N/V1)*y[3])-((k12N/V2)*y[28]); //(in ng/s) given y[3] & y[28] are (in ng), V1 & V2 are (in ml), k12 is (in ml/s)
	//====================================================Antagonist PK (Method 2)

	//Opioid PK (Method 2)===================================================
	/*r(DoseCompartment) (in ng/s) given y[5] is (in ng), opioidTransferRate is (in 1/s), y[33] is (in -)*/
	ydot[5] = -(opioidTransferRate*y[33]*y[5])-((1-y[33])*opioidTransferRate*y[5]);
	/*r(TransferCompartment1) (in ng/s) given y[6] is (in ng), opioidTransferRate is (in 1/s), y[33] is (in -)*/
	ydot[6] = (opioidTransferRate*y[33]*y[5])-(opioidTransferRate*y[6]);
	/*r(TransferCompartment2) (in ng/s) given y[6] & y[7] are (in ng), opioidAbsorptionRate & opioidTransferRate are (in 1/s)*/
	ydot[7] = (opioidTransferRate*y[6])-(opioidAbsorptionRate*y[7]);
	/*r(CentralCompartment) (in ng/s) given y[7], y[8] & y[4] are (in ng), opioidAbsorptionRate is (in 1/s), opioidCentralVolume & opioidPeripheralVolume are (in ml), opioidClearanceRate, opioidInterCompartmentalClearanceRate are (in ml/s)*/
	ydot[8] = (opioidAbsorptionRate*y[7])-((opioidClearanceRate/opioidCentralVolume)*y[8])- ((opioidInterCompartmentalClearanceRate/opioidCentralVolume)*y[8])+((opioidInterCompartmentalClearanceRate/opioidPeripheralVolume)*y[31]);
	/*r(PeripheralCompartment) (in ng/s) given y[8] & y[4] are (in ng), opioidCentralVolume & opioidPeripheralVolume are (in ml), opioidInterCompartmentalClearanceRate is (in ml/s)*/
	ydot[31] = ((opioidInterCompartmentalClearanceRate/opioidCentralVolume)*y[8])-((opioidInterCompartmentalClearanceRate/opioidPeripheralVolume)*y[31]);
	/*r(EffectSite) (in pM/s) given y[32] is (in pM), opioidKeo is (in 1/s), y[8] is (in ng), opioidCentralVolumeModified is (in ml), opioidMolarMass is (in g/mol)*/
	ydot[32] = (opioidKeo*((y[8]*1e3)/(opioidCentralVolumeModified/1e3))/opioidMolarMass)-(opioidKeo*y[32]); //with cardiovascular compensation
	//	ydot[32] = (opioidKeo*((y[8]*1e3)/(opioidCentralVolume/1e3))/opioidMolarMass)-(opioidKeo*y[32]); //without cardiovascular compensation
	/*r(Bioavailability) (in 1/s)*/
	ydot[33] = 0;
	//====================================================Antagonist PK (Method 2)

	//	//Opioid PK====(Method 2)
	//	ydot[5] = -(koutC*y[5])-(k12C*y[5])+(k21C*y[6])+(k31C*y[7])-(k13C*y[5])+y[23]/Infu_t; //(in mg/s)
	//	ydot[6] = (k12C*y[5])-(k21C*y[6]); //(in mg/s)
	//	ydot[7] = (k13C*y[5])-(k31C*y[7]); //(in mg/s)
	//	ydot[8] = (k1*((y[5]*1e9)/VPC)/Mmass)-(k1*y[8]); //(in pM/s)

//	ydot[9] = (A1*(1-y[9]-y[10])*(pow(y[8],n)))-(B1*y[9]);
	ydot[9] = (A1*(1-y[9]-y[10])*(pow(y[32],n)))-(B1*y[9]);
	...
	ydot[27] =0; /* */
	ydot[29] =ReactionFlux25Child; /* dyo2Child */ //added//
	ydot[30] = 0; /* New bioavailability Term */
	...
	//	yout[9]=((y[8]*1e6)/(VPC*1e3)); /*Opioid plasma concentration (ng/mL)*/
	yout[9]=(y[8]/opioidCentralVolumeModified); /*Opioid plasma concentration (ng/mL)*/ //with cardiovascular compensation
	//	yout[9]=(y[8]/opioidCentralVolume); /*Opioid plasma concentration (ng/mL)*/ //without cardiovascular compensation
	yout[10]=(y[3]/V1C); /*Antagonist plasma concentration (ng/mL)*/
	yout[11]=y[32]/1e3; /*Opioid effect site concentration (nM)*/
	...
	yout[16]=arterialOxygenSaturation; /*Arterial O2 saturation (%)*/
	//	yout[17]=VPC; /*Opioid volume of distribution (L?)*/
	yout[17]=opioidCentralVolumeModified; /*Opioid volume of distribution (L?)*/
	yout[18]=V1C;
	...
	yout[29]=P_T_o2;
	yout[30]=P_T_co2;
	yout[31]=arterialOxygenSaturation2; /*Arterial O2 saturation (%) alternate*/
}
