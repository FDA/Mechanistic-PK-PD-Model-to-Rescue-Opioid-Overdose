...
void derivs (int *neq, double *t, double *y, double *ydot, double *yout, int *ip){
	...
	double* intermediate_list= calc_intermediate(y[11],y[12],y[13],y[14],y[15],y[16],y[18],y[19],y[25],y[26],y[26],y[28]);
	...
	double ReactionFlux25Child=1.00/tau_o2Child*(psai_o2Child - y[28]); //unchanged//
	...
	ydot[0] = -(ktr*y[29]*y[0])-((1-y[29])*ktr*y[0]); //(in ng/s) given y[0] is (in ng), ktr is (in 1/s), y[29] is (in -)
	ydot[1] = (ktr*y[29]*y[0])-(ktr*y[1]); //(in ng/s) given y[1] is (in ng), ktr is (in 1/s), y[29] is (in -)
	ydot[2] = -(1*kin*y[2])+(ktr*y[1]); //(in ng/s) given y[1] & y[2] are (in ng), kin & ktr are (in 1/s)
	ydot[3] = (1*kin*y[2])-((kout2/V1)*y[3]); //(in ng/s) given y[2], y[3] are (in ng), kin is (in 1/s), V1 is (in ml), kout2 is (in ml/s)
	...
	//	//	ydot[8] = (k1*1e6*y[5]*1000.00/VPC/Mmass)-(k1*y[8]); //(in pM/s)
	...
	ydot[27] =0; /* */
	ydot[28] =ReactionFlux25Child; /* dyo2Child */ //added//
	ydot[29] = 0; /* New bioavailability Term */
	ydot[30] = 0; /* blank */
