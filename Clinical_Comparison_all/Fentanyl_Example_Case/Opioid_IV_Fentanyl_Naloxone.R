...
p5<-p5+ylab(paste0("Fractional Ventilation"))+xlab(paste0("time(min)"))+theme(legend.position = "none") 
...
p5<-p5+ylab("Ventilation (% of baseline)") +
		xlab("Time (minutes)") +
		theme_bw() +
		theme(legend.direction = "vertical",
				legend.position = c(0.8, 0.8),
...
p5<-p5+geom_text(x=150,y=0,label="Naloxone")
