...
void derivs (int *neq, double *t, double *y, double *ydot, double *yout, int *ip){
	...
	ydot[2] = -(1*kin*y[2])+(ktr*y[1]); //(in ng/s) given y[2] & y[1] are (in ng), kin & ktr are (in 1/s)
	ydot[3] = (1.00/V1C)*((1*kin*y[2])-(kout2*y[3])- (k12N*y[3]))+1/V1C*((k12N*y[28])); //(in ng/ml/s) given y[3] & y[28] are (in ng/ml), V1C is (in ml), kout2 & k12N are (in ml/s), y[2] is (in ng), kin is (1/s)
	ydot[28] = 1/V2*((k12N*y[3]))-1/V2*((k12N*y[28])); //(in ng/ml/s) given y[3] & y[28] are (in ng/ml), k12N is (in ml/s), V2 is (in ml)
	ydot[4] = (k2*y[3]*1e6/molarMassAntagonist)-(k2*y[4]); //(in pM/s) given y[4] is (in pM), k2 is (in 1/s), y[3] is (in ng/ml or ug/L), molarMassAntagonist is (in g/mol)
	//=========================================================Antagonist PK (Method 1)
	...
	yout[10]=y[3]; /*Antagonist plasma concentration (ng/mL)*/
