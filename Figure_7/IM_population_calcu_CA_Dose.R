...
#	opioid="carfentanil"
	Type="chronic"
	if(Product=="EVZIO2"){
	fo=sprintf("final4_%s_opt_yesParDis_noDelayDiss_conc%s_PK%s_%s_%s_samplingD_log_Nal0_I",Product,fo1,PK_yn,opioid,Type)
...
#}
	...
	k1111<-0
for (i in 1:2000) {
	if (file.exists(sprintf("MCMC_outputs/%s/All%s.csv",folder_of_population,i))) {
	d0=read.csv(sprintf("MCMC_outputs/%s/All%s.csv",folder_of_population,i),stringsAsFactors =F)
	k1111<-k1111+1
	if(d0[2,2]=="no"){
	d2<-d0
...
populationFolder=sprintf("MCMC_outputs/%s/results/",folder_of_population)
system(paste0("mkdir -p ","MCMC_outputs/final2"))
