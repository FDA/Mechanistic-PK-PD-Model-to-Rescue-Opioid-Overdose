patientWrapper<-function(patientidx,doseidx, max_naloxone_doseN=6,naloxone_dose,delay, threshold,dose_time=dose_times){
	...
	dosingoutput<-dosingevents(opioid_dose,opioid_time=dose_time) 
	print(tail(fulltimes))
...
#	write.csv(eventdata,"NEWeventdata.csv")
#	write.csv(Preout1,"NEW1outiman.csv")
#	write.csv(fulltimes,"NEW1fulltimes.csv")
#	
#	eeeeeeeeee
...
#out <- fundedewithEvent(states, fulltimes, truepar, namesyout,eventdata)
	...
	if (dose_times>0){crossinglist<-300} #predosign  
	if (dose_times==0){try({crossinglist <- crossing(patientventi,out[,"time"],threshold)},silent=T)} 
...
#	print(t1P)
	...
	if(!exists("crossinglist")){  #no crossing
		return(list(outlist,ventilist))
		crossingtime<-120 # wanted us to administer naloxone even if we didn't hit threshold
	}else{
		
		if (dose_times>0){crossingtime <- 0} #predosign  (also initial delay =0s)
		if (dose_times==0){crossingtime <- crossinglist[[2]][1]}  #normal administration (also initial delay =60s)
#		crossingtime <- 0#crossinglist[[2]][1]
		crossingtimelist[[1]]<-crossingtime
		rm(out)                     #release memory
		rm(crossinglist)
	}
	...
	for(naloxone_doseN in 1:max_naloxone_doseN){
		...
		dosingoutput<-dosingevents(opioid_dose, opioid_time=dose_time,naloxone_time=crossingtime+this.par["initialdelay"], naloxone_doseN=naloxone_doseN,gap=delay)
...
#		write.csv(P_thrsh_O2,sprintf("iman_crosscount/%s_%s.csv",naloxone_doseN,ipop))
		P_thrsh_CO2=Preout1[,c("time","Arterial CO2 partial pressure (mm Hg)")][Preout1[,c("Arterial CO2 partial pressure (mm Hg)")]>=P_a_co2_thrsh ,] #store values of time and PaCO2 for times where PaCO2 > PaCO2 threshold so times where collapse should happen due to PaCO2 
		if (length(P_thrsh_O2)!=0) { #if there are times where PaO2 can cause collapse
			truepar["CA_delay_o2"]=P_thrsh_O2[1,"time"]+delaytime #calculate and store the time where collpase should happen due to PaO2 in truepar
		}else{print("O2 doesn't cross the threshold")}
...
#			colapse_p=Preout1[(Preout1[,"time"])==(truepar["CA_delay_o2"]),"Arterial O2 partial pressure (mm Hg)"] #get PaO2 value at the time collpase is supposed to start
		...
		#Iman added (check it by John)
		if (truepar["CA_delay_o2"]>max(fulltimes)) {colapse_p=100000} # means if colapedhappen after our max run time then we dont care
#		print("-----------**********************")	
#		print((Preout1[,"time"]))
#		print("-----------k")
#			
##			print(fulltimes)
#			print(round(truepar["CA_delay_o2"],1))
#			
#		print("colapse_p-0--------------")
#		print(P_thrsh_O2)
#		print(colapse_p)
#		
#		print("P_a_o2_thrsh_p-0--------------")
#		print(P_a_o2_thrsh)
...
#			crossingtimelist[[naloxone_doseN+1]]<- 0#crossinglist[[2]][2]-crossinglist[[2]][1]+this.par["initialdelay"]
			if (dose_times==0){crossingtimelist[[naloxone_doseN+1]]<-crossinglist[[2]][2]-crossinglist[[2]][1]+this.par["initialdelay"]} 
			
			}#if crossing
		}
			if (dose_times>0){crossingtimelist[[naloxone_doseN+1]]<- 0} #predosign  
