fundedewithEvent<-function(states, fulltimes,truepar,namesyout,eventdata){
	...
	try({out <- dede(states, fulltimes, "derivs", parms=truepar, dllname="delaymymod",initfunc="initmod", nout=22, rtol=1e-10, atol=1e-10, method="adams",n_history = 100000,events=list(data=eventdata))});
