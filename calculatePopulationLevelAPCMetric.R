...
#define inputs=====================================================================
APCQuantilesToReport=c(2.5/100, 5/100, 25/100, 50/100, 1-25/100, 1-5/100, 1-2.5/100)
#==================================================================================
#loading libraries====
...
if (inputs$antagonistAdministrationRouteAndDose== "IN3nalmefeneB"){
	...
	antagonistDoseLabel="Intranasal nalmefene C 3 mg C"
...
if (inputs$antagonistAdministrationRouteAndDose== "OpioidOnly"){
	...
	inputFolderAPC= sprintf("output/%s/individualSubjects%s/%s_%s_%s_%s_%s_%s_%s_%s,%smins_APCOpioidOnly", 
			...
			inputs$subjectAge,
			simulationTime) 
	inputFolderAPCTypicalSubject= sprintf("output/%s/individualSubjects%s/%s_%s_%s_%s_%s_%s_%s_%s,%smins_APCOpioidOnly", 
			inputs$antagonistAdministrationRouteAndDoseTemp, 
	...
	inputFolderAPC=				sprintf("output/%s/individualSubjects%s/%s_%s_%s_%s_%s_%s_%s_%s,%smins_APC",
			...
			inputs$subjectAge,
			simulationTime) 
	inputFolderAPCTypicalSubject= sprintf("output/%s/individualSubjects%s/%s_%s_%s_%s_%s_%s_%s_%s,%smins_APC", 
			inputs$antagonistAdministrationRouteAndDose, 
			inputs$typicalSubjectInputDate,		
			inputs$opioid, 
			inputs$opioidDose,
...
if (inputs$antagonistAdministrationRouteAndDose== "OpioidOnly"){
	inputs$antagonistAdministrationRouteAndDoseTemp= "IN3nalmefeneB"
	outputFolder=sprintf("output/%s/populationOutput%s/%s_%s_%s_%s_%s_%s_%s_%s,%smins_APCOpioidOnly", 
			inputs$antagonistAdministrationRouteAndDoseTemp, 
			Sys.Date(), 
			inputs$opioid, 
			inputs$opioidDose, 
	...
	outputFolder=sprintf("output/%s/populationOutput%s/%s_%s_%s_%s_%s_%s_%s_%s,%smins_APC", 
			inputs$antagonistAdministrationRouteAndDose, 
			Sys.Date(), 
			inputs$opioid, 
			inputs$opioidDose, 
...
print("STARTING APC TIMECOURSE PROCESSING===========================================================================================")
...
getAPCData <- function(inputFolder, subjectIndex){
	print(subjectIndex)
...
getAllSubjectData <- function(inputFolder, n){
	return(data.frame(rbindlist(lapply(1:n, function(x) getAPCData(inputFolder, x)))))
}
allSubjectData=getAllSubjectData(inputFolderAPC, inputs$numberOfSubjects)
...
typicalSubjectData=getAPCData(inputFolderAPCTypicalSubject, 2001)
...
write.csv(combinedTypicalAndPopulationData, sprintf("%s/APC_%s_%s.csv", outputFolder, inputs$numberOfSubjects, inputs$PercentCI))
#=========================================================================calculate mean, median, quantiles from data of all subjects
print("FINISHED APC TIMECOURSE PROCESSING===========================================================================================")
#====================================================================read data for all subjects and calculate mean, median, quantiles
...
plotUpToTime<-function(timeToPlot=c()){
	...
	ggsave(sprintf("%s/APC_%s_%s_%s.jpg", outputFolder, inputs$numberOfSubjects, inputs$PercentCI, timeToPlot), plot, height=2.5, width=3)
}
