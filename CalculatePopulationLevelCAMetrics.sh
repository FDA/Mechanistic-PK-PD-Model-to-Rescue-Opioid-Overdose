...
#SBATCH -J CPLCAM								# Name the job as 'MPItest'
#SBATCH --account=CDERID0047
#SBATCH --output=output/logfiles/CPLCAM_%j.out	# Write the standard output to file
...
Rscript calculatePopulationLevelCAMetric.R  -a "$opioid"   -b "$dose"  -c "$antagonist" -d "$antagonistAdministrationRouteAndDose" -r "$inputDate"